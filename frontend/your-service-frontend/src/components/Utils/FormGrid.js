...
const InputField = styled.input`
  flex: 1 1 calc(50% - 20px); /* Ajuste para que os campos ocupem metade da largura com espaço entre eles */
...
const FormGrid = ({ title, fields, submitText, additionalLinks, endpoint }) => {
  ...
  return (
    <FormContainer onSubmit={handleSubmit}>
      ...
      {fields.map(field => (
        formErrors[field.name] && <ErrorMsg key={field.name}>{formErrors[field.name]}</ErrorMsg>
      ))}
      {formSubmitted && fields.map(field => (
        !formErrors[field.name] && <SuccessMsg key={field.name}><FiCheckCircle size={18} /> {field.label} válido</SuccessMsg>
      ))}
